...
function parseToIR(content: string, fmt: string): IRNode[] {
  ...
  switch (f) {
    ...
    case 'markdown': case 'html': { for(const line of content.split('\n')){const m=line.match(/^(#{1,6})\s+(.+)/);if(m){nodes.push({type:'heading',level:m[1]!.length,value:m[2]!.trim()})}else if(line.match(/^[-*+]\s+/)){const last=nodes[nodes.length-1];if(last?.type==='list')last.items!.push(line.replace(/^[-*+]\s+/,''));else nodes.push({type:'list',items:[line.replace(/^[-*+]\s+/,'')]})}else if(line.trim()){nodes.push({type:'paragraph',value:line.trim()})}} break }
...
function renderIR(nodes: IRNode[], target: string): string {
  const out: string[] = []
  switch (target) {
    case 'json': { const j:Record<string,unknown>={}; for(const n of nodes){switch(n.type){case 'heading':j['h'+n.level]=n.value;break;case 'paragraph':j['p']=n.value;break;case 'list':j['list']=n.items;break;case 'table':j.table={h:n.headers,r:n.rows};break;case 'code':j.code=n.lines?.join('\n');break;default:j.text=n.lines?.join('\n')}} return JSON.stringify(j,null,2) }
    case 'csv': { const rows:string[][]=[]; for(const n of nodes){if(n.type==='heading')rows.push(['h'+n.level,n.value??'']);else if(n.type==='paragraph')rows.push(['text',n.value??'']);else if(n.type==='list')n.items?.forEach(i=>rows.push(['item',i]));else if(n.type==='table'&&n.headers){rows.push(n.headers);n.rows?.forEach(r=>rows.push(r))}else if(n.type==='code'||n.type==='raw')n.lines?.forEach(l=>rows.push(['text',l]))} return rows.map(r=>r.join(',')).join('\n') }
    ...
    case 'tsv': { const rows:string[][]=[];for(const n of nodes){if(n.type==='heading')rows.push(['heading',String(n.level??''),n.value??'']);else if(n.type==='paragraph')rows.push(['text',n.value??'']);else if(n.type==='list')n.items?.forEach(i=>rows.push(['item',i]));else if(n.type==='table')n.headers&&rows.push(n.headers),n.rows?.forEach(r=>rows.push(r));else if(n.type==='code'||n.type==='raw')n.lines?.forEach(l=>rows.push(['text',l]))} return rows.map(r=>r.join('\t')).join('\n') }
...
export function universalConvert(content: string, sourceFormat: string, targetFormat: string): string {
  try { if (isBrowserConvertible(sourceFormat, targetFormat)) return convertInBrowser(content, sourceFormat, targetFormat) } catch {}
  const actual = sourceFormat === 'unknown' ? detectFormatFromContent(content) : sourceFormat
  try { if (actual !== sourceFormat && isBrowserConvertible(actual, targetFormat)) return convertInBrowser(content, actual, targetFormat) } catch {}
